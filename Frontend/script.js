...
document.querySelectorAll(".skills-heading, .skill").forEach((el) => {
  el.dataset.animation = "fadeInUp 1.5s ease-out forwards";
  el.style.animation = el.dataset.animation;
  el.style.animationPlayState = "paused";
  observer.observe(el);
    ...
    try {
      ...
      comments.forEach((comment) => {
        ...
        div.innerHTML = `
          ...
          <div class="reactions">
            <button onclick="react('${comment._id}', 'like')">👍 ${
          comment.reactions?.like || 0
        }</button>
            <button onclick="react('${comment._id}', 'love')">❤️ ${
          comment.reactions?.love || 0
        }</button>
            <button onclick="react('${comment._id}', 'laugh')">😂 ${
